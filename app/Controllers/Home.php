...
class Home extends BaseController
{
    ...
    public function index(): string
    {
        ...
        $notices = $db->table('notices')
            ->select('id, title, type, date, file_path, external_link')
            ->where('status', 1)
            ->distinct()
        ...
        // Format date and create fallback href
        foreach ($notices as &$notice) {
            $notice['date_formatted'] = date('d M Y', strtotime($notice['date']));
        ...
        // 🔥 NEW: Fetch homepage sliders
        $sliderModel = new HomeSliderModel();
        $homeSliders = $sliderModel
            ->where('status', 1)
            ->orderBy('sort_order', 'asc')
            ->findAll();
        ...
        $branches = $branchModel->where('status', 1)
            ->orderBy('ifsc', 'asc')
            ->limit(5)
            ->findAll();
        ...
        // Fetch Payment Settlement PDF from downloads table
$downloadModel = new \App\Models\DownloadModel();
$paymentSettlement = $downloadModel
    ->where('title', 'Payment Settlement Contact Details')
    ->where('status', 1)
    ->first();

$data['paymentSettlementPdf'] = $paymentSettlement['file_path'] ?? '';

// ========== APPLY NOW PANEL SETTINGS ==========
$applyPanelSettingModel = new \App\Models\ApplyPanelSettingModel();
$applyPanelSettings = $applyPanelSettingModel->getSettings();

// Fetch active apply panel items
$applyPanelItemModel = new \App\Models\ApplyPanelItemModel();
$applyPanelItems = $applyPanelItemModel->getActiveItems();
        ...
        $data = [
            ...
            'applyPanelSettings' => $applyPanelSettings,
'applyPanelItems' => $applyPanelItems,
        ];
