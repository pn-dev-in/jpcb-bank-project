    ...
    <div class="col-xl-3 col-md-6">
        <div class="card bg-warning text-white">
            <div class="card-body">
                ...
                <h2 class="mt-2 mb-0"><?= $pending ?? 0 ?></h2>
            </div>
            <div class="card-footer bg-transparent border-top-0">
    ...
    <div class="col-xl-3 col-md-6">
        <div class="card bg-success text-white">
            <div class="card-body">
                ...
                <h2 class="mt-2 mb-0"><?= $resolved ?? 0 ?></h2>
            </div>
            <div class="card-footer bg-transparent border-top-0">
    ...
    <div class="col-xl-3 col-md-6">
        <div class="card bg-danger text-white">
            <div class="card-body">
                ...
                <h2 class="mt-2 mb-0"><?= $delayed ?? 0 ?></h2>
            </div>
            <div class="card-footer bg-transparent border-top-0">
...
<!-- ========== 2. CRITICAL ALERTS ========== -->
<?php if (($unassigned ?? 0) > 0 || ($olderThan24h ?? 0) > 0): ?>
...
<!-- ========== 3. RECENT COMPLAINTS ========== -->
<div class="card mb-4">
    <div class="card-header py-2"><i class="fas fa-table me-1"></i> Recent Complaints</div>
                ...
                <tbody>
                    <?php if (!empty($recentComplaints)): ?>
                        <?php foreach ($recentComplaints as $c): ?>
                            <tr>
                                <td><?= esc((string)($c['ticket_number'] ?? 'N/A')) ?></td>
...
<td><?= esc((string)($c['assigned_admin_name'] ?? 'Unassigned')) ?></td>
<td><?= date('d M Y H:i', strtotime($c['created_at'] ?? 'now')) ?></td>
<td><a href="<?= base_url('admin/complaints/view/'.($c['id'] ?? 0)) ?>" class="btn btn-sm btn-primary">View</a></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
...
<div class="row g-3 mb-4">
    <div class="col-xl-6 col-md-6">
        <div class="card h-100">
            <div class="card-header py-2"><i class="fas fa-chart-pie me-1"></i> Admin Workload (Unresolved)</div>
            <div class="card-body p-0">
                <?php if (!empty($assignmentOverview)): ?>
                    <div class="list-group list-group-flush">
                        <?php foreach ($assignmentOverview as $admin): ?>
                            <div class="list-group-item d-flex justify-content-between align-items-center">
                                <?= esc((string)($admin['name'] ?? 'Unknown')) ?>
    ...
    <div class="col-xl-6 col-md-6">
        <div class="card h-100">
            <div class="card-header py-2"><i class="fas fa-chart-line me-1"></i> Analytics (Last 30 Days)</div>
            <div class="card-body">
                <div><strong>Complaints this week:</strong> <?= $complaintsThisWeek ?? 0 ?></div>
                <div class="mt-2"><strong>Complaints this month:</strong> <?= $complaintsThisMonth ?? 0 ?></div>
                <div class="mt-2">
                    <strong>Resolution rate (month):</strong> <?= $resolutionRate ?? 0 ?>%
                    <div class="progress mt-1" style="height: 6px;">
                        <div class="progress-bar bg-success" style="width: <?= $resolutionRate ?? 0 ?>%"></div>
                    </div>
                </div>
                <div class="mt-2"><strong>Average resolution time:</strong> <?= $avgResolution ?? 0 ?> hours</div>
                <div class="mt-2">
                    <strong>SLA compliance (48h):</strong> <?= $slaCompliance ?? 0 ?>%
                    <div class="progress mt-1" style="height: 6px;">
                        <div class="progress-bar bg-info" style="width: <?= $slaCompliance ?? 0 ?>%"></div>
                    </div>
                </div>
            </div>
        </div>
...
<!-- ========== 5. ACTIVITY LOG ========== -->
<div class="card mb-4">
    <div class="card-header py-2"><i class="fas fa-history me-1"></i> Recent Activity Log</div>
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-sm table-hover mb-0">
                <thead>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5" class="text-center">No activity recorded.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div class="row g-3">
    <div class="col-md-6">
        <div class="card h-100">
            <div class="card-header py-2"><i class="fas fa-shield-alt me-1"></i> Security Info</div>
            <div class="card-body">
                <p><strong>Last login:</strong> <?= $lastLogin ?? 'Never' ?></p>
                <p><strong>Failed login attempts (your account):</strong> <?= $failedAttempts ?? 0 ?></p>
            </div>
        </div>
    </div>
    <div class="col-md-6">
        <div class="card h-100">
            <div class="card-header py-2"><i class="fas fa-bell me-1"></i> Notifications</div>
            <div class="card-body">
