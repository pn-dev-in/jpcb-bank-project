        ...
        <form method="post" action="<?= isset($item) ? base_url('admin/digisaathi-contacts/update/'.$item['id']) : base_url('admin/digisaathi-contacts/store') ?>">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <small class="text-muted">e.g., phone, message-circle, globe, message-square</small>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Type</label>
                    <select name="type" class="form-control" required>
                        <option value="phone" <?= (isset($item) && $item['type']=='phone') ? 'selected' : '' ?>>Phone</option>
                        <option value="call" <?= (isset($item) && $item['type']=='call') ? 'selected' : '' ?>>Call</option>
                        <option value="whatsapp" <?= (isset($item) && $item['type']=='whatsapp') ? 'selected' : '' ?>>WhatsApp</option>
                        <option value="website" <?= (isset($item) && $item['type']=='website') ? 'selected' : '' ?>>Website</option>
                        <option value="chatbot" <?= (isset($item) && $item['type']=='chatbot') ? 'selected' : '' ?>>Chatbot</option>
                    </select>
                </div>
            </div>
            <div class="row mb-3">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="button_text" class="form-control" value="<?= old('button_text', $item['button_text'] ?? '') ?>" placeholder="e.g., Call Now, Message Us">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Button Color (optional)</label>
                    <input type="text" name="button_color" class="form-control" value="<?= old('button_color', $item['button_color'] ?? '') ?>" placeholder="e.g., primary, success, danger">
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label">Link (URL)</label>
                <input type="text" name="link" class="form-control" value="<?= old('link', $item['link'] ?? '') ?>" required>
                <small class="text-muted">e.g., tel:14431, https://wa.me/918928913333</small>
            </div>
            <div class="mb-3">
                <label class="form-label">Image Path (optional)</label>
                <input type="text" name="image" class="form-control" value="<?= old('image', $item['image'] ?? '') ?>" placeholder="assets/img/contact-image.jpg">
            </div>
            <div class="row mb-3">
                <div class="col-md-4">
