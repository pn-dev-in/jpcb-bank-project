...
<div class="card">
    ...
    <div class="card-body">
        <form method="post"
            action="<?= isset($item) ? base_url('admin/downloads/update/' . $item['id']) : base_url('admin/downloads/store') ?>"
            ...
            <div class="mb-3">
                <label class="form-label">Document File
                    <?= isset($item) ? '(optional, leave empty to keep current file)' : '*' ?></label>
                <?php if (isset($item) && !empty($item['file_path'])): ?>
                    <div class="mb-2">
                        <a href="<?= base_url($item['file_path']) ?>" target="_blank" class="text-primary">Current file:
                            <?= basename($item['file_path']) ?></a>
                        <p class="text-muted small mt-1">Upload a new file to replace it.</p>
                    </div>
                <?php endif; ?>
                <input type="file" name="document" class="form-control" accept=".pdf,.doc,.docx,.xls,.xlsx"
                    <?= !isset($item) ? 'required' : '' ?>>
                <small class="text-muted d-block">Allowed types: PDF, DOC, DOCX, XLS, XLSX. Max size: 10MB.</small>
            </div>

            <div class="mb-3">
    <label class="form-label">Cover Image / Thumbnail
        <?= isset($item) ? '(optional, leave empty to keep current)' : '(optional)' ?></label>
    <?php if (isset($item) && !empty($item['thumbnail'])): ?>
        <div class="mb-2">
            <img src="<?= base_url($item['thumbnail']) ?>" alt="Current thumbnail"
                 style="max-height:120px;border-radius:6px;border:1px solid #dee2e6;">
            <p class="text-muted small mt-1">Upload a new image to replace it.</p>
        </div>
    <?php endif; ?>
    <input type="file" name="thumbnail" class="form-control" accept="image/*">
    <small class="text-muted d-block">Accepted: JPG, PNG, WEBP. Recommended: 400×550px (portrait/A4 ratio).</small>
</div>
